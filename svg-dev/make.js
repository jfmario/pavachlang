...
let vowels = require('./vowels.json');
...
// matching direction
function transformVowel1(x, y, dir=1) {
...
// mismatching direction
function transformVowel2(x, y, dir=1) {
...
function processVowel(letterKey) {
  
  let letter = vowels[letterKey];
  let re = /[A-Z,][\s\d]+/g;
    ...
    if (m) {
      ...
      if (splitString.length % 2 != 0) {
        ...
        while (splitString.length > 0) {
          ...
          let t1 = transformVowel1(x, y, letter.direction);
          let t2 = transformVowel2(x, y, letter.direction);
...
function main() {
  ...
  let outVowels = {};
  for (let letterKey in vowels) {
    outVowels[letterKey] = processVowel(letterKey);
  }
  
  let outfile1 = path.resolve(__dirname, 'site', 'consonants.json');
  fs.writeFileSync(outfile1, JSON.stringify(outConsonants));
  
  let outfile2 = path.resolve(__dirname, 'site', 'vowels.json');
  fs.writeFileSync(outfile2, JSON.stringify(outVowels));
}
