      ...
      for (let j = 0; j < words.length; ++j) {
        ...
        for (let k = 0; k < characters.length; ++k) {
          ...
          if (character.indexOf('.') == -1) {
            this.pathlist.push(this.consonants[character].standard);
          }
          else {
            if (character.startsWith('.')) {
              this.pathlist.push(this.vowels[character.slice(1)].standard);
            }
            else {
              ...
              let c = subs[0];
              let v = subs[1];
              let consonant = this.consonants[c];
              let vowel = this.vowels[v];
